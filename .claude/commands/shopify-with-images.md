...
Use this command to create a Shopify-ready HTML article, generate the configured
number of relevant images, insert them into the article, and upload the HTML and
image assets to Google Cloud Storage.
...
2. Creates article-relevant images with OpenAI image generation
3. Uploads generated images to Google Cloud Storage
...
Generate the configured number of images. The default is two:
- One hero-style editorial image near the introduction
- One supporting in-article image near a later section
- Additional images, when requested, should support later article sections
...
- Additional requested images follow the same `image-[number].png` pattern
